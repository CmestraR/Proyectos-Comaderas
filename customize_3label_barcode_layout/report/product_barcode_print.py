...
class TRReportBarcodeLabels(models.AbstractModel):
    _name = 'report.customize_3label_barcode_layout.report_product_trbarcode'
    _description = 'barcode report TR'

    @api.model
    def _get_report_values(self, docids, data=None):
        browse_record_list = []
        product_obj = self.env["product.product"]
        config = self.env.ref('customize_3label_barcode_layout.default_trbarcode_configuration')
        if not config:
    ...
    def _get_barcode_string(self, barcode_value, data):
        ...
        barcode_str = "<img style='width:" + str(data['display_width']) + "px;height:" + str(
            data['display_height']) + "px'src='data:image/png;base64,{0}'>".format(encoded_string)
        return barcode_str or ''
    ...
    def _divided_blank_update(self, total_quantities):
        ...
        """
        lists = []
        needs_to_add = total_quantities % 3
        if needs_to_add == 1:
            lists.append({'name_1': ' '})
            lists.append({'name_2': ' '})
            lists.append({'name_3': ' '})
    ...
    def _get_lines(self, form):
        ...
        for l in lines:
            ...
            for c in range(0, int(l['qty'])):
                ...
                dict_data.update({
                    'name' + '_' + str(value): len(p.name) > 30 and (p.name[:30]) or p.name[:30],
                    'code' + '_' + str(value): barcode_value,
                    'variants' + '_' + str(value): len(attribute_string) > 30 and (attribute_string[:30]) or attribute_string[:30] or '',
                    'default_code' + '_' + str(value): p.default_code or '',
                    'barcode' + '_' + str(value): p.barcode or '',
                })
                total_value += 1
                if total_value % 3 == 0:
