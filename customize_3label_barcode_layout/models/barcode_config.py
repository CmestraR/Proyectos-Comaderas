...
class TRBarcodeConfiguration(models.Model):
    ...
    def apply(self):
        format = self.env.ref('customize_3label_barcode_layout.paperformat_trbarcode_label')
        if format:
            format.sudo().write({
    ...
    @api.model
    def get_config(self):
        return self.env.ref('customize_3label_barcode_layout.default_trbarcode_configuration')

class CssCompany(models.Model):
