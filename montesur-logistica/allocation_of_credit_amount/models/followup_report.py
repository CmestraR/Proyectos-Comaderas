...
class AccountFollowupReport(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        ...
        if self.env.context.get('print_mode'):
            # Remove the 'Expected Date' and 'Excluded' columns
            headers = headers[:7] + headers[9:]
        return headers
    ...
    def _get_lines(self, options, line_id=None):
        ...
        partner = options.get('partner_id') \
            and self.env['res.partner'].browse(options['partner_id']) or False
        if not partner:
            return []

        lang_code = partner.lang if self._context.get('print_mode') \
            else self.env.user.lang or get_lang(self.env).code
        lines = []
        res = {}
        today = fields.Date.today()
        line_num = 0
        for line in partner.unreconciled_aml_ids.filtered(
        ...
        for currency, aml_recs in res.items():
            ...
            for aml in aml_recs:
                amount = aml.amount_residual_currency \
                    if aml.currency_id else aml.amount_residual
                date_due = format_date(self.env, aml.date_maturity
                                       or aml.date, lang_code=lang_code)
                ...
                lines.append({
                    ...
                    'unfoldable': False,
                    'columns': [
                        type(v) == dict and v or {'name': v} for v in columns],
                })
    ...
    def _build_followup_summary_with_field(self, field, options):
        ...
        :param options: dict that should contain the followup
                        level and the partner
        ...
        if followup_line:
            ...
            try:
                ...
                summary = summary % {
                    ...
                    'partner_identification': partner.number_identification
                    or partner.vat or '',
                ...
                message = _("An error has occurred while formatting your "
                            "followup letter/email. (Lang: %s, Followup "
                            "Level: #%s) \n\nFull error description: %s") \
                          % (lang, followup_line.id, exception)
                raise ValueError(message)
            return summary
        raise UserError(_('You need a least one follow-up level in order to '
                          'process your follow-up'))
        ...
        for line in self:
            if line.description:
                try:
                    ...
                    raise Warning(_('Your description is invalid, use the '
                                    'right legend or %% if you want to use '
        ...
        for line in self:
            if line.sms_description:
                try:
                    ...
                    raise Warning(_('Your sms description is invalid, use the '
                                    'right legend or %% if you want to use '
                                    'the percent character.'))
