...
class TestAccountFollowup(TestAccountNoChartCommon):
    ...
    def test_05_followup_multicompany(self):
        ...
        invoice_move = self.env['account.move'].with_context(default_type='out_invoice').create({
            ...
            'invoice_line_ids': [
                (0, 0, {'quantity': 1, 'price_unit': 30, 'tax_ids': self.tax_id.ids}),
            ],
        })
        ...
        invoice_move1 = self.env['account.move'].with_context(default_type='out_invoice').create({
            ...
            'invoice_line_ids': [
                (0, 0, {'quantity': 1, 'price_unit': 60,'tax_ids': self.tax_id.ids}),
            ],
        })
        ...
        self.assertEqual(self.partner_customer_usd.credit, 35.7)
        ...
        self.assertEqual(lines[1]['columns'][8]['name'], formatLang(self.env, 35.7, currency_obj=currency))

        # For company 1
        currency = company1.currency_id
        self.assertEqual(self.partner_customer_usd.with_context(allowed_company_ids=company1.ids).credit, 71.4)
        ...
        self.assertEqual(lines[1]['columns'][7]['name'], 'Total Due')
        self.assertEqual(lines[1]['columns'][8]['name'], formatLang(self.env, 71.4, currency_obj=currency))
        ...
        invoices = self.env['account.move'].create([
            {
                ...
                'type': 'out_invoice',
                'invoice_line_ids': [(0, 0, {'quantity': 1, 'price_unit': 40, 'tax_ids': self.tax_id.ids})],
            },
            {
                'partner_id': test_partner.id,
                'invoice_date': today + relativedelta(days=-11),
                'type': 'out_invoice',
                'invoice_line_ids': [(0, 0, {'quantity': 2, 'price_unit': 40, 'tax_ids': self.tax_id.ids})],
            },
        ])
        invoices.post()
    ...
    def test_followup_level_and_status(self):
        ...
        ten_days_ago_invoice = self.env['account.move'].create({
            ...
            'type': 'out_invoice',
            'invoice_line_ids': [(0, 0, {'quantity': 1, 'price_unit': 30, 'tax_ids': self.tax_id.ids})]
        })
        ...
        forty_days_ago_invoice = self.env['account.move'].create({
            ...
            'type': 'out_invoice',
            'invoice_line_ids': [(0, 0, {'quantity': 1, 'price_unit': 20, 'tax_ids': self.tax_id.ids})]
        })
...
class TestAccountFollowupReports(TestAccountReportsCommon):
    ...
    def test_followup_report_initial_state(self):
        ...
        print(report._get_lines(options))
    ...
    def _create_payment(env, date, invoices, amount=None, journal=None):
        ...
        '''
        group_id = env['res.groups'].search([
            ('name','=','ACE_CARTERA Y TESORERIA')
        ], limit=1)
        if group_id:
            env.user.write({
                'groups_id': [(4, group_id.id, 0)],
            })
